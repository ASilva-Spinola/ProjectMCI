...
[Simulation MCIs Coimbra](docs/SimulationMCIsCoimbra.csv) has 564 individuals -a subset with longitudinal data-. These are classified according to their *diagnosis* (DX) according to their total score on the Clinical Dementia Rating (CDR) as: neurological controls (NC), amnesic MCI (EMCI), non-amnesic MCI (LMCI) and AD.
